...
class ScraperTestCase(TestCase):
    ...
    @patch("kyiv_weather.scraper.Weather.objects.update_or_create")
    @patch("kyiv_weather.scraper.parse_weather")
    def test_save_weather_raises_error_with_invalid_data(
        self, mock_parse, mock_update_or_create
    ):
        mock_weather = Mock(spec=Weather, date="invalid_date")
        mock_parse.return_value = [mock_weather]
        mock_update_or_create.side_effect = ValidationError("Invalid data")
